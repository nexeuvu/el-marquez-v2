    ...
    <div class="w-full bg-zinc-900 rounded-xl shadow-2xl overflow-hidden p-6 border border-zinc-800">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-white">
                Lista de Servicios de Habitación
            </h1>
            <div class="space-x-2">
                <a href="{{ route('admin.room_service.export-pdf') }}"
                    class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Exportar PDF
                </a>
                <a href="{{ route('admin.room_service.export-excel') }}"
            ...
            <table class="w-full">
                <thead class="bg-zinc-800">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-zinc-300 uppercase">Tipo de Servicio</th>
                    ...
                    @foreach ($room_services as $service)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-sm text-zinc-300">
                                {{ $service->employee->name ?? 'N/D' }} {{ $service->employee->last_name ?? '' }}
                            </td>
                            <td class="px-4 py-4 text-sm text-zinc-300">
                                Habitación {{ $service->room->number ?? 'N/D' }}
                            </td>
                            <td class="px-4 py-4 text-sm text-zinc-300">
                            ...
                            <td class="px-4 py-4 text-sm text-right">
                                {{-- Editar --}}
                                <a href="{{ route('admin.room_service.update', $service->id) }}"
                                ...
                                <form id="delete-form-{{ $service->id }}"
                                    action="{{ route('admin.room_service.destroy', $service->id) }}"
        ...
        {{-- Paginación --}}
        @if ($room_services instanceof \Illuminate\Pagination\LengthAwarePaginator)
            <div class="mt-6">
                {{ $room_services->links() }}
            </div>
        @endif
